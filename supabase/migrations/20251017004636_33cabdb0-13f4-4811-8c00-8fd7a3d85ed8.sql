-- Update admin_approve_payment to handle referral rewards
  ...
  if v_res.type = 'subscription_request' then
    ...
    if v_friend_code is not null then
      ...
      if v_code_owner_id is not null then
        ...
        -- Send notification to code owner
        insert into public.notifications (user_id, role, title, body)
        values (
          v_code_owner_id,
          'user',
          'Código de Convite Usado!',
          '1 amigo usou o teu código, tens 25% desconto ativo! Aproveita e marca uma sessão!'
        );
