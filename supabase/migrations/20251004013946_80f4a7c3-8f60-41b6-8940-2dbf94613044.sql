-- Corrigir apply_offer para usar o total_duration_min da oferta (180min) no snapshot
...
BEGIN
  ...
  -- Criar a reserva com o total_duration_min da oferta (180min = 3h) no snapshot
  ...
  VALUES (
    ...
    v_starts_at, 
    v_ends_at,
    v_offer_record.total_duration_min, -- 180 min (3h totais: 2h pagas + 1h grátis)
    v_offer_record.price_eur, -- 20 EUR
    'EUR',
    v_offer_record.name || ' (' || v_offer_record.total_duration_min || 'min totais)', -- Nome descritivo
    'pending'
  )
