-- Create a dedicated subscription service for FK constraint
DO $$
DECLARE
...
BEGIN
  ...
  IF v_subscription_service_id IS NULL THEN
    ...
    VALUES (
      ...
      'Serviço placeholder para subscrições',
      0,
      1,  -- Minimum duration to satisfy constraint
      'EUR',
      false  -- Not active so it doesn't show in public lists
    )
    RETURNING id INTO v_subscription_service_id;
  END IF;
END $$;

-- Update subscribe_request to use the subscription service
