...
-- 1. Atualizar o texto da notificação na função admin_approve_payment
-- Vou recriar a função com o novo texto da notificação
  ...
  if v_res.type = 'subscription_request' then
    ...
    if v_friend_code is not null then
      ...
      if v_code_owner_id is not null then
        ...
        -- NOVO TEXTO DA NOTIFICAÇÃO
        insert into public.notifications (user_id, role, title, body)
        values (
          v_code_owner_id,
          'user',
          'Código de Convite Usado!',
          '1 amigo usou o teu código, tens 25% desconto ativo! Aproveita!'
        );
