-- Update admin_approve_payment to use extensions.http_post instead of net.http_post
...
begin
  ...
  if v_res.type = 'subscription_request' then
    ...
    if v_transfer_link is not null and v_service_name ilike '%Mix%Master%' then
      -- Call edge function to send email with notes using extensions.http_post
