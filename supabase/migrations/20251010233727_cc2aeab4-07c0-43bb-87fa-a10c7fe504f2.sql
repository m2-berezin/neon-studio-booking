-- Update admin_approve_payment to NOT delete reservations after approval
-- This allows Mix & Master projects to remain visible in admin dashboard
...
begin
  ...
  if v_res.type = 'subscription_request' then
    ...
    -- REMOVED: delete from public.reservations where id = v_res.reservation_id;
    -- Reservations are now kept so they remain visible in admin projects page
...
-- Create function to delete Mix & Master project
...
begin
  ...
  -- Get reservation_id from payment_request
