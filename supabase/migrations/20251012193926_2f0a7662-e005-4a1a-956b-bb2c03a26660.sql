-- Update admin_approve_payment to reset loyalty points when approving a loyalty offer
...
begin
  ...
  if v_res.type = 'subscription_request' then
    ...
    -- ADD LOYALTY POINTS for Mix & Master services (but NOT for loyalty offers)
    ...
    -- RESET LOYALTY POINTS for loyalty offers (delete 7 points)
    if v_is_loyalty_offer then
      -- Delete the oldest 7 loyalty points for this user
