...
-- Update admin_approve_payment to create referral reward when referee uses any points
...
begin
  ...
  IF v_points_used > 0 THEN
    ...
    -- Check if the referee (current user) used someone's referral code
    ...
    AND fcu.used_in_payment = false  -- Not yet rewarded
    LIMIT 1;

    -- If referee used a referral code and hasn't triggered reward yet
    IF v_referee_code_use_id IS NOT NULL AND v_referrer_user_id IS NOT NULL THEN
      -- Check if reward was already given for this code use
      SELECT EXISTS(
      ...
      -- Only create reward if not already given
      IF NOT v_reward_already_given THEN
        -- Give 2500 GW to the referrer
        INSERT INTO public.point_transactions (
          ...
          2500,
          'referral_earned',
          'Recompensa por convite - amigo utilizou GW',
        ...
        SET 
          ...
          payment_request_id = p_payment_id
        WHERE id = v_referee_code_use_id;

        -- Create referral_reward record for tracking
        ...
        INSERT INTO public.notifications (
          ...
          'user',
          '💎 Recebeste 2500 GW!',
          'O teu amigo ' || COALESCE(v_client_name, 'um utilizador') || ' utilizou GW numa reserva! Recebeste 2500💎 GW como recompensa.'
        );
      END IF;
    END IF;
  END IF;
...
-- Retroactive fix: Give Marcos his 2500 GW reward since teste2 used GW
DO $$
DECLARE
  v_referee_code_use_id uuid;
  v_referrer_user_id uuid;
  v_referee_name text;
BEGIN
  -- Find the code use by teste2
  SELECT fcu.id, fc.created_by, p.full_name
  INTO v_referee_code_use_id, v_referrer_user_id, v_referee_name
  FROM public.friend_code_uses fcu
  JOIN public.friend_codes fc ON fc.code = fcu.code
  JOIN public.profiles p ON p.id = fcu.used_by
  WHERE fcu.id = 'f5207f20-b2d3-4cdf-ae51-1d191a5971cc';
  ...
  IF v_referee_code_use_id IS NOT NULL AND v_referrer_user_id IS NOT NULL THEN
    ...
    IF NOT EXISTS(
      ...
      INSERT INTO public.point_transactions (
        ...
        2500,
        'referral_earned',
        'Recompensa por convite - amigo utilizou GW (correção retroativa)',
        v_referee_code_use_id
      );
      ...
      INSERT INTO public.notifications (
        ...
        'user',
        '💎 Recebeste 2500 GW!',
        'O teu amigo ' || COALESCE(v_referee_name, 'um utilizador') || ' utilizou GW numa reserva! Recebeste 2500💎 GW como recompensa.'
      );
    END IF;
  END IF;
END $$;
