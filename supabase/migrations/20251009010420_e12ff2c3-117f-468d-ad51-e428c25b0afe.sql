-- Fix http_post call with correct signature
...
begin
  ...
  if v_res.type = 'subscription_request' then
    ...
    if v_transfer_link is not null and v_service_name ilike '%Mix%Master%' then
      -- Call edge function to send email using http_post with correct signature
      ...
      raise notice 'Email send response: %', v_http_response;
    end if;
