-- Fix admin_approve_payment to remove FOR UPDATE with LEFT JOIN and include notes in email
...
begin
  ...
  -- Get payment request without FOR UPDATE to avoid conflict with LEFT JOIN
  ...
  if v_res.type = 'subscription_request' then
    ...
    if v_transfer_link is not null and v_service_name ilike '%Mix%Master%' then
      -- Call edge function to send email with notes
