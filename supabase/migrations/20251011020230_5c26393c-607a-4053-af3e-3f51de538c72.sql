-- Fix admin_approve_payment to update existing subscription instead of creating new one
...
BEGIN
  ...
  IF v_res.type IN ('subscription', 'subscription_request') THEN
    ...
    -- Use the subscription_id from the payment request (created by subscribe_request)
    v_subscription_id := v_res.subscription_id;

    IF v_subscription_id IS NOT NULL THEN
      -- Update the existing subscription created by subscribe_request
    ...
    ELSE
      -- Fallback: if no subscription_id, find or create subscription
      ...
      IF v_subscription_id IS NOT NULL THEN
        ...
        WHERE id = v_subscription_id;
      ELSE
        -- Create new subscription (should rarely happen)
