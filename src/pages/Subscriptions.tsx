...
const Subscriptions = () => {
  ...
  useEffect(() => {
    const fetchActivePlan = async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('subscriptions').select('plan_type').eq('user_id', user.id).eq('is_active', true).single();
      if (data && !error) {
  ...
  const subscriptionPlans = [{
    ...
    price: 4.99,
    duration: 'monthly',
    features: ['10% desconto em todos os serviços no primeiro mês', '15% desconto nos meses seguintes', 'Ao fim de 6 meses: oferta de 1 Mix&Master'],
    popular: false
  }, {
    id: 'plan-x',
    name: 'Plano X',
    ...
    price: 9.99,
    duration: 'monthly',
    features: ['10% desconto no primeiro mês em todos os serviços', '15% desconto nos meses seguintes', 'Oferta de 2h de captação por mês', 'Ao fim de 6 meses: 1 captação mix e master'],
    popular: true
  }];
  ...
  const handleDayToggle = (dayValue: string) => {
    const newDays = tempPreferences.preferred_days.includes(dayValue) ? tempPreferences.preferred_days.filter(d => d !== dayValue) : [...tempPreferences.preferred_days, dayValue];
    setTempPreferences(prev => ({
      ...prev,
      preferred_days: newDays
    }));
  };
  const handleTimeWindowToggle = (timeWindow: {
    start: string;
    end: string;
  }) => {
    const isSelected = tempPreferences.preferred_time_windows.some(tw => tw.start === timeWindow.start && tw.end === timeWindow.end);
    const newTimeWindows = isSelected ? tempPreferences.preferred_time_windows.filter(tw => !(tw.start === timeWindow.start && tw.end === timeWindow.end)) : [...tempPreferences.preferred_time_windows, timeWindow];
    setTempPreferences(prev => ({
      ...prev,
      preferred_time_windows: newTimeWindows
    }));
  };
  const startEditing = () => {
  ...
  return <div className="space-y-6">
      ...
      {userSubscription && <section>
          <div className="flex items-center justify-between mb-4">
            ...
            {!editingPreferences && <Button variant="outline" onClick={startEditing}>
          ...
          <Card className="studio-card">
            ...
            <CardContent>
              {editingPreferences ? <div className="space-y-6">
                  {/* Preferred Days */}
                  <div>
                    <Label className="text-base font-semibold mb-3 block">Dias Preferidos</Label>
                    <div className="grid grid-cols-3 gap-3">
                      {daysOfWeek.map(day => <div key={day.value} className="flex items-center space-x-2">
                          <Checkbox id={`day-${day.value}`} checked={tempPreferences.preferred_days.includes(day.value)} onCheckedChange={() => handleDayToggle(day.value)} />
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {timeWindows.map((timeWindow, index) => <div key={index} className="flex items-center space-x-2">
                          <Checkbox id={`time-${index}`} checked={tempPreferences.preferred_time_windows.some(tw => tw.start === timeWindow.start && tw.end === timeWindow.end)} onCheckedChange={() => handleTimeWindowToggle(timeWindow)} />
                  ...
                  {preferences.preferred_days.length > 0 ? <>
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {preferences.preferred_days.map(dayValue => <Badge key={dayValue} variant="secondary">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {preferences.preferred_time_windows.map((timeWindow, index) => <Badge key={index} variant="secondary">
      ...
      {/* Suggested Slots */}
      {userSubscription && suggestedSlots.length > 0 && <section>
          <h2 className="text-2xl font-bold text-foreground mb-4">Suggested Slots</h2>
          <div className="grid md:grid-cols-2 gap-4">
            {suggestedSlots.map((slot, index) => <Card key={index} className="studio-card">
          ...
          <Card className="studio-card bg-gradient-to-br from-primary/10 to-accent/10">
            ...
            <CardContent>
              <div className="flex items-center justify-between">
                ...
                <Button onClick={requestMonthlySchedule} disabled={loading} className="ml-4">
